...
(() => {
 'use strict';
 const kind=document.documentElement.dataset.tool;
 const dictionary={
 ...
 short:{title:'Long address. Short story.',intro:'Turn a lengthy URL into a neat link that is easy to share.',generate:'Shorten link',privacy:'Your address is stored on this server for 365 days. Do not submit confidential links. No account needed; up to 20 new links per hour per IP address.',emptyTitle:'Less link. More possibility.',emptyText:'Your new link will appear here after you shorten an address.',resultCaption:'Your short link',step2Text:'We save your redirect for 365 days.',step3Text:'Copy your link or turn it into a QR code.'},
 ...
 short:{title:'Lange Adresse. Kurze Geschichte.',intro:'Verwandle eine lange URL in einen kompakten Link, den du einfach teilen kannst.',generate:'Link kürzen',privacy:'Die Adresse wird 365 Tage auf diesem Server gespeichert. Keine vertraulichen Links eingeben. Ohne Konto; bis zu 20 neue Links pro Stunde und IP-Adresse.',emptyTitle:'Weniger Zeichen. Mehr Möglichkeiten.',emptyText:'Dein neuer Link erscheint hier nach dem Kürzen.',resultCaption:'Dein kurzer Link',step2Text:'Die Weiterleitung bleibt 365 Tage gespeichert.',step3Text:'Kopiere den Link oder erstelle einen QR-Code.'},
 ...
 function localize(){
  ...
  document.title=(kind==='qr'?'QR Studio':'Link Studio')+' — '+t[kind==='qr'?'qrTab':'shortTab'];
  document.querySelectorAll('.tool-tabs a').forEach(a=>{const u=new URL(a.href);u.searchParams.set('lang',lang);a.href=u.href;});
  const home=lang==='pl'?'../index.html':'../index-'+lang+'.html';$('portfolio-link').href=home;$('back-link').href=home+'#projects';
 ...
 function setQrLink(){const u=new URL('qr.html',location.href);u.searchParams.set('lang',lang);u.hash=new URLSearchParams({url:currentUrl}).toString();$('make-qr').href=u.href;}
 ...
 $('tool-form').addEventListener('submit',async event=>{
  ...
  try{
   const response=await fetch('api.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url}),signal:controller.signal});
   const data=await response.json();if(!response.ok)throw Error(data.error);
   if(!/^[\w-]{8}$/.test(data.code))throw Error('unavailable');
   if(version!==requestVersion)return;
   const result=new URL('../s.php',location.href);result.searchParams.set('c',data.code);expiry=data.expires;
